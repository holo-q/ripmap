...
Distinguish between 'signal loss' (ranker too strict) and 'noise injection' (ranker too loose). If 'depth' failures persist, distinguish 'unreachable' from 'undervalued'. If targets are missing, distinguish 'unreachable' (disconnected) from 'drowned' (rank too low). If drowned by hubs, REDUCE alpha to sharpen local signal. Only increase alpha if the graph is genuinely fragmented.
...
- Deep files (e.g. `src/impl/deep.rs`) are often reachable via 'Test <-> Impl' temporal coupling even when structurally distant. Use temporal boosts to bridge directory gaps.
